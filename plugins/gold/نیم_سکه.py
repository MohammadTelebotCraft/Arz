...
async def handle_gold(event, client):
    """Handle نیم سکه gold type requests"""
    ...
    gold_info = next((g for g in gold_types if g['currencyName'] == 'نیم سکه'), None)
    if not gold_info:
        await event.respond('اطلاعات نیم سکه در حال حاضر در دسترس نیست. ❌')
        return
