...
class UserDatabase:
    ...
    def add_user(self, user_id: int, username: Optional[str] = None, 
        ...
        try:
            ...
            if result:
                # User exists, update last_seen and increment interaction_count
    ...
    def log_user_action(self, user_id: int, action: str) -> bool:
        ...
        try:
            ...
            # Add a user_stats entry for this interaction
            self.cursor.execute('''
                INSERT INTO user_stats (user_id, action, timestamp)
                VALUES (?, ?, ?)
            ''', (user_id, action, current_time))
            
            # Update the user's last_seen time and increment interaction_count
            self.cursor.execute('''
                UPDATE users 
