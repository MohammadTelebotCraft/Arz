...
async def handle_gold(event, client):
    """Handle انس طلا gold type requests"""
    ...
    gold_info = next((g for g in gold_types if g['currencyName'] == 'انس طلا'), None)
    if not gold_info:
        await event.respond('اطلاعات انس طلا در حال حاضر در دسترس نیست. ❌')
        return
