...
async def handle_gold(event, client):
    """Handle ربع سکه gold type requests"""
    ...
    gold_info = next((g for g in gold_types if g['currencyName'] == 'ربع سکه'), None)
    
    if not gold_info:
        await event.respond('اطلاعات ربع سکه در حال حاضر در دسترس نیست. ❌')
        return
